...
tree_elem_t* createNode (tree_t* rTree)
{
    tree_elem_t* addedElement = (tree_elem_t*) calloc (1, sizeof (tree_elem_t));
    assert (addedElement);

    addedElement->parent = NULL;
    addedElement->left   = NULL;
    addedElement->right  = NULL;
    addedElement->type   = NOTH;
    addedElement->data   = POISON;
...
int fPrintfElem (tree_elem_t* element, FILE* ptrFile)
{
    ...
    {
        fprintf (ptrFile, "elem%d [label=\"var№ %lg\"]", i, element->data);
    }
    else if (element->type == ASSIGN)
    {
        fprintf (ptrFile, "elem%d [label=\"assign \\nvar№ %lg \"]", i, element->data);
    }
    else if (element->type == CALL)
    {
        fprintf (ptrFile, "elem%d [label=\"call \\nfunc№ %lg \"]", i, element->data);
    }
    else if (element->type == FUNCTION)
    {
        fprintf (ptrFile, "elem%d [label=\"func№ %lg \"]", i, element->data);
    }
    #define DEF_OPER(name, text, command)                       \
        else if (element->type == name )                        \
        {                                                       \
            fprintf (ptrFile, "elem%d [label="#text"]", i);     \
        }
